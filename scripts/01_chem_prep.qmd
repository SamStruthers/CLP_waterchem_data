...
newest_raw_file <- "ROSS_20240423.csv"
```
...
new_field_meas <- read_csv('data/raw/ROSS_field_meas_upto_2024-04-16.csv', show_col_types = FALSE)%>%
...
mutate(final_Field_DO_mgL = case_when( 
      ...
      final_Field_Cond_µS_cm = case_when( 
          ...
          final_Field_Temp_C = case_when( !is.na(temp_c) ~ temp_c, 
                                  ...
                                  TRUE ~ NA), 
      final_time_mst = case_when( !is.na(time_mst) ~ time_mst,
...
rm(new_field_meas, old_field_meas, site_meta, water_chemistry_import, newest_raw_file, buffer_sbs, dist_from_pbd, watershed_sbs)
...
write_csv_arrow(select(water_chemistry_tidy, -DT_mst),paste0("data/cleaned/CLP_chemistry_up_to_",up_to_date, ".csv") )

# Write to RDS file as well
write_rds(x = water_chemistry_tidy, file = paste0("data/cleaned/CLP_chemistry_up_to_",up_to_date, ".rds"))
